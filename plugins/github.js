...
var activityQueries = {
  ...
  async getByContributor(db, username, limit) {
    let sql = "SELECT * FROM activity WHERE contributor = ? ORDER BY occured_at DESC";
  ...
  async getByDateRange(db, startDate, endDate) {
    const result = await db.execute("SELECT * FROM activity WHERE occured_at >= ? AND occured_at <= ? ORDER BY occured_at DESC", [startDate, endDate]);
    return result.rows.map(parseActivity);
  },
  /**
  ...
  async getByDefinition(db, definitionSlug) {
    const result = await db.execute("SELECT * FROM activity WHERE activity_definition = ? ORDER BY occured_at DESC", [definitionSlug]);
    return result.rows.map(parseActivity);
  },
  /**
  ...
  async getByDefinitions(db, activityDefinitionSlugs) {
    ...
    const result = await db.execute(`SELECT * FROM activity 
       WHERE activity_definition IN (${placeholders})
       ORDER BY occured_at ASC`, activityDefinitionSlugs);
    return result.rows.map(parseActivity);
  },
  /**
  ...
  async getByContributorAndDefinitions(db, contributor, activityDefinitionSlugs) {
    ...
    const result = await db.execute(`SELECT * FROM activity 
       ...
       ORDER BY occured_at ASC`, [contributor, ...activityDefinitionSlugs]);
    return result.rows.map(parseActivity);
  },
  /**
  ...
  async getTotalPointsByContributor(db, username) {
    const result = await db.execute("SELECT COALESCE(SUM(points), 0) as total FROM activity WHERE contributor = ?", [username]);
...
function activitiesFromIssues(issues, repo) {
  ...
  for (const issue of issues) {
    ...
    for (const assignEvent of issue.assign_events) {
      ...
      if (lastestIssueAssignEvents[slug] && new Date(lastestIssueAssignEvents[slug].occured_at) > new Date(assignEvent.createdAt)) {
        continue;
      }
